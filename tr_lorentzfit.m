function [A,m,s,b]=tr_lorentzfit(X0,X,Y,LB,UB,autoback)
...
%actual regression
lorentz=@(F,x) (F(1)*0.5*F(3)./((((x-F(2)).^2)+((0.5*F(3))^2))))+F(4);
% options = optimset('Display','off');
% 
% [x,resnorm] = lsqcurvefit(@myfun,x0,xdata,ydata,[],[],options);

F=lsqcurvefit(lorentz,X0,X,Y,LB,UB,optimset('maxfunevals',10000,'MaxIter',10000,'Display','off'));
A=F(1);
m=F(2);
s=F(3);
b=F(4);

 plot(X,lorentz(F,X),'-r')
 pause
 close
close
end