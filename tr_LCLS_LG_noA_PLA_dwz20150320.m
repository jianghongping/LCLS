...
dark=dir('Dark/*dark*');
x=importdata(strcat('Dark/',dark.name));
...
for k = 1 : imagenum
    %Get immediate high intensity peak for calculation fo offset theta
    %redo
     offset_raz(k)=raz(k);%get_peak_index(sliceint10,3,4)+beamstop_edge_index(k);
...
%   display which image is being read
    disp(['Integrating LY ', imagelist(k).name, ' at correct Sami orientation.']);
...
%     end
    ...
    if length(sliceint10r)~=length(qvec)
        filld=zeros(length(qvec),1);
        if length(sliceint10r)<length(qvec)
            filld(1:length(sliceint10r),1)=sliceint10r(1:length(sliceint10r));
        elseif length(sliceint10r)>length(qvec)
            filled(1:length(qvec),1)=sliceint10r(1:length(qvec));
        end
...
%     peakloc(k) = 355;
    ...
    [hpatched,hfit,az_patched,az_fit] = tr_hermans_masked_withfit(az1);
    herm(k) = hpatched;
    hermfit(k)=hfit;
    storeaz(k).theta = az1(:,1)+offset_theta(k);
    storeaz(k).laz = az_patched(:,2);
    storeaz(k).lazfit=az_fit(:,2);
%     plotPause=0;
%     while plotPause==0
