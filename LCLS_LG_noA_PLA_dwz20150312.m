...
sampprefix = 'LCLS-run14saxs-overview-masked-test';
fpath = 'output';
mkdir(fpath,sampprefix)
fpath=strcat(fpath,'/',sampprefix,'/');
sampprefix = strcat(fpath,'/',sampprefix,'-');
mask=dir('Masks'); %originally mask=dir('Masks/*saxs*');
mask.mask=importdata(strcat('Masks/',ds2_2-mask.mat)); %strcat('Masks/',mask.name));
SDD=2243;       %LCLS CXI SAXS detector
...
%     herm_start = pi;
%     herm_end = 3*pi/2;
    thetamin=80*pi/180;     %LCLS CXI (7/22/14)
    thetamax=100*pi/180;    %LCLS CXI (7/22/14)
    herm_start=pi/2;        %LCLS CXI (7/22/14)
...
%   display which image is being read
    ...
    [qvec, sliceint10, darclengths, gaps]=wedge_lineout_masked(saxsi(k).images,saxsi(k).energy,SDD,pix, [xc, yc], thetamin,...
        thetamax, thetasteps, rmax, offset_theta(k));
    
    J=1;
    while (gaps(J+1)-gaps(J))==1
...
%     peakloc(k) = 355;
    ...
    az1 = azimuthal_subtract(saxsi(k).images, xc, yc, dtheta, peakloc(k), window, theta_start, theta_end);
...
for k = 1: imagenum
    ...
    az1 = azimuthal_masked(saxsi(k).images, xc, yc, dtheta, raz(k)-beamstop_edge_index(k), window, theta_start, theta_end); 
    storeaz(k).theta = az1(:,1);
    storeaz(k).laz = az1(:,2);
