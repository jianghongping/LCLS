...
limit.D = [8 11];
limit.sD = [1.5 2.5];
limit.c = [0.6 0.8]; % c applies only if t is not specified
...
[storeim saxsi] = read_all_images_new(imagelist,imagenum,ccd_type,beamstop_position);

%%%%ADDED IN OCT 2011%
%total intensity analysis
 
 minGI = -.02; % in degrees
 stepGI = 0.02; % in degrees
 maxGI = 0.4; % in degrees
 ...
 for k = 1: imagenum
     ...
     TOTALintensity (k) = sum(sum(saxsi(k).images));
 ...
 disp('Optimum incidence angle is');
 disp(' ');
 disp(rangeGI(MAXintens_I));
