...
%Tom Riis: Import Dark image to be subtracted from images
dark=dir('Dark/*dark*');
...
%Tom Riis: Go through all images in run files within image run folder
...
elseif beamstop_position == 1 % center
    ...
    thetamax=10*pi/180;    
    herm_start=0;
    herm_end=pi/2;             %LCLS CXI (7/22/14)
else % left
    thetamin = -10*pi/180;
    thetamax = 10*pi/180;
    herm_start = pi/2;
...
for k=1:length(saxsi)
    ...
    %SUBTRACT Background by subtracting dark image
%     figure
...
%     figure
%     imagesc(saxsi(k).images)
%     set(gca,'DataAspectRatio',[1,1,1])
%     title(strcat(imagelist(k).name,' Dark Subtracted'))
%     hold on
...
%     imagesc(saxsi(k).images)
%     set(gca,'DataAspectRatio',[1,1,1])
%     title(strcat(imagelist(k).name,'Normalized'))
%     hold on
end
...
count=1;
increment=10;
for i=increment:increment:length(saxsi)
    [SaxsiAverage(count),StoreIm(count)]=SaxsiInterval(saxsi,storeim,[start,i]);
...
saxsi=SaxsiAverage;
storeim=StoreIm;
imagenum=length(saxsi);

for i=1:length(saxsi)
    imagelist(i).name=strcat('ds-',num2str(saxsi(i).interval(1)),'-',num2str(saxsi(i).interval(2)),'.mat');
    holder(i)=imagelist(i);
end
imagelist=holder;
offset_theta=zeros(1,imagenum);
for k=1:length(saxsi)
    storeim(k).images=saxsi(k).images;
end
%%%%GET SCATTERING INTENSITIES%%%%%
for i=1:length(storeim)
    ScatteringI(i)=sum(sum(saxsi(i).images));
end
%heights (last image coordinate = 0)
height=[saxsi.y]-saxsi(end).y;

%heights First image is coordinate =0
%height=saxsi(1).y-[saxsi.y];
...
fitchoice ='h'; %input('Diameter and hermans [d], hermans only [h], or screening [else]? > ','s');
disp(' ');
...
%end
    ...
    offchoice = 0;%input('Auto theta-offset [1] or auto [0/else]? > ','s');
...
%   display which image is being read
    ...
    [qvec, sliceint10, darclengths, gaps]=tr_wedge_lineout_masked_strict(saxsi(k).images,saxsi(k).energy,SDD,pix, [xc, yc], thetamin,...
    ...
    raz(k) = tr_get_peak_index(qvec,sliceint10,lowerPeakBound,upperPeakBound)+beamstop_edge_index(k);
...
for k = 1 : imagenum
    offset_raz(k)=raz(k);%get_peak_index(sliceint10)+beamstop_edge_index(k);
    offset_theta(k) = get_offset_theta_redo(storeim(k).images, imagelist(k).name, saxsi(k).images,...
        xc, yc, offset_raz(k), dtheta, window, ...
...
%   display which image is being read
    ...
    [qvecr, sliceint10r, darclengthsr, gapsr]=tr_wedge_lineout_masked_strict(saxsi(k).raw,saxsi(k).energy,SDD,pix, [xc, yc], thetamin,...
        thetamax, thetasteps, rmax, offset_theta(k));
...
%     end
    ...
    ivsq(k).dataNoNorm=[qvec', sliceint10];
    if length(sliceint10r)~=length(sliceint10)
        %dispp=length(sliceint10)-length(sliceint10r)
        filld=zeros(length(sliceint10),1);
        filld(1:length(sliceint10r),1)=sliceint10r;
        ivsqRAW(k).data=[qvec',filld];
    else
        ivsqRAW(k).data=[qvec',sliceint10r];
    end
...
if fitchoice == 'h'
    ...
    peakloc(k) = tr_get_peak_pos(dataq, dataI, raz(k)-beamstop_edge_index(k)); %Tom Riis: changed to angstroms
%     peakloc(k) = 355;
    humpqs(k) = qvec(peakloc(k));

    theta_start = herm_start + offset_theta(k);
    theta_end = herm_end + offset_theta(k);
    az1 = azimuthal_masked(saxsi(k).images, xc, yc, dtheta, peakloc(k), window, theta_start, theta_end); %Tom Riis: changed function from masked_strict version
    az1(:,1)=az1(:,1)-offset_theta(k); 
    if ((thetamin+thetamax)/2)==pi/2
        az1(:,1)=abs(az1(:,1)-pi/2);
    end
    [hpatched,hfit,az_patched,azfit] = tr_hermans_masked_withfit(az1);
    herm(k) = hpatched;
    hermfit(k)=hfit;
    storeaz(k).theta = az1(:,1)+offset_theta(k);
    storeaz(k).laz = az_patched(:,2);
    storeaz(k).lazfit=azfit(:,2);
%     plotPause=0;
%     while plotPause==0
%         figure; plot(az_patched(:,1),az_patched(:,2),'y*');hold on;plot(az1(:,1),az1(:,2),'r*')
%         title('Intensity vs. phi'); xlabel('phi [radians]'); ylabel('Intensity [a.u.]');
%         plotPause=str2double(input('Az_patched (yellow) vs Az_Masked (red)  > ','s'));
%     end
    end
    ...
    % plot all results (no fitting)
     for i=1:imagenum
        index=find(ivsqRAW(i).data(:,1)==humpqs(i));
        peakIntensity(i)=ivsqRAW(i).data(index,2);
    end
    disp('single_manual_plot_hermans')
    single_manual_plot_hermans(storeim, imagelist,height,...
         x1, imagenum, humpqs, storeaz, herm,hermfit,...
         ivsq, offset_theta,ScatteringI,peakIntensity,saxsi, xc, yc, sampprefix,offset_raz);
     
   
    tr_savedata2Excel(sampprefix,ivsq,storeaz,herm,hermfit,offset_theta,ScatteringI,peakIntensity,humpqs,saxsi,imagelist,imagenum);
    % save fit parameters
    % columns
    % h H peakloc
...
for k = 1 : imagenum
    ...
    while ~isempty(go)
        ...
        theta_start = herm_start + offset_theta(k);
        theta_end = herm_end + offset_theta(k);
        az1 = azimuthal_masked(saxsi(k).images, xc, yc, dtheta, peak, window, theta_start, theta_end); %Tom Riis: changed function from maxked_strict version
        ...
        title('Intensity vs. q'); xlabel('q [A^{-1}]'); ylabel('Intensity [a.u.]');
        axis tight;
        hold off;
        findfigs
...
disp('single_manual_plot_fitting')
...
for k = 1: imagenum
    ...
    az1 = azimuthal_masked(saxsi(k).images, xc, yc, dtheta, raz(k)-beamstop_edge_index(k), window, theta_start, theta_end); %Tom Riis: changed function from version azimutal masked_strict
    storeaz(k).theta = az1(:,1);
    storeaz(k).laz = az1(:,2);
...
% plot all results (no fitting)
disp('single_manual_plot_no_fitting')
